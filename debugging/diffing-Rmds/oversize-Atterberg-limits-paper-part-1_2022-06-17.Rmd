...
All analyses were performed using the `lm()` function in the R Language for Statistical Computing (version `r stringr::str_extract(sessionInfo()$R.version$version.string, "(?<=R\\sversion\\s)[\\dd\\.]*(?=\\s)")`) [@RCoreTeam2022].


The analyses also utilized the authors’ soiltestr package for R (<https://github.com/evanmascitti/soiltestr>), which was developed using principles outlined within the tidyverse framework [@Wickham2019a].
...
A `Makefile` which runs the relevant analysis code is included in the supplemental materials.
...
For example, adding 70% coarse additions in the 2000-1000 &mu;m fraction produced estimated $T_{max}$ of `r very_coarse_sand_t_max_at_70_pct_sand` kJ m^-3^, while 70% 250-150 &mu;m or 150-53 &mu;m reduced $T_{max}$ to zero.
...
In the present study, SSA for each coarse addition was computed using 2.65 Mg m^3^ for $\rho_p$ and taking the representative particle diameter as the D~50~ of each material.
