...
<!-- via washing. -->
This directive makes it difficult to compare mixes with different sand gradations because variation among two soils' plasticity may be confounded by removing different amounts of sand.
...
This research suggests particles 2000-425 &mu;m may remain in infield mixes before Atterberg limit tests.
<!-- need not be removed -->
...
@Casagrande1932 deemed the upper and lower limits of dense liquid too difficult to define and that the sticky and cohesion limits pertained more to the workability of clay in ceramics than to agriculture or earthworks engineering.
...
Atterberg limit tests are performed only on material \<425 &mu;m sieve diameter [@ASTMD43182018; @AASHTO2020a].
Particles >425 &mu;m are termed "oversize" by ASTM D4318 [@ASTMD43182018].
If oversize particles cannot be removed by hand, the standard dictates the sample be wet-sieved over a \<425 &mu;m sieve.


The authors are not aware of a fundamental rationale for the choice of 425&mu;m as the upper boundary for Atterberg limit tests, although we may conjecture that it was chosen to match the boundary between 'medium' and 'fine' sand in Casagrande's later-published Airfield Classification (AC) scheme [@Casagrande1942].
...
Therefore, when oversize particles are removed to conduct the test, the obtained LL or PL must be adjusted to estimate the whole-soil LL or PL.
...
The the linear law of mixtures (or more simply, linear law) is analogous to the correction performed after removal of naturally-occurring particles >425 &mu;m (Equation \@ref(eq:oversize-correction-formula)).
...
For soils with LL <35, allowing the 2000-425 mu&;m particles to remain in the sample resulted in minimal LL differences between sample treatment methods. 
...
The infield skin is composed of compacted bare soil and is considered the most critical portion of the field [@Zwaska2007b].
The behavior of an infield soil is markedly affected by its water content [@STMA2015].
...
Infield skin soils are often dry screened to pass a 2000 &mu;m sieve, but they may still contain up to 50% of their total mass between 2000 &mu;m and 425 &mu;m [@Schroder2012].
...
If the linear correction is not used, the removal of particles >425 &mu;m may limit comparison of test results. Consider a case in which two soils contain identical total sand content but different types of fines and different sand-size gradations. These soils will likely have different LL and PL values; however, it will be unclear whether the differences are due to the nature of the fines, or simply because more sand 425-2000 &mu;m was removed from the soil having the finer sand gradation.
...
Gravity sedimentation plus centrifugation were used along with the the pipette method to compute the percent of the sample finer than 20, 5, and 2 μm [@Gee2002]. 
...
The component percentages were adjusted for the trace amounts of particles between 2000-53 &mu;m in the clay soil and the air-dry water content of each component.
Final mixture percentages are expressed on an oven-dry basis (kg kg^-1^).
...
The D~50~ values in Table \@ref(tab:usga-sieve-sizes-table) were determined graphically [@USGA2018] from Figure \@ref(fig:experiment-1-particle-size-curves).
...
The analyses also utilized the authors’ soiltestr package for R (<https://github.com/evanmascitti/soiltestrhttps://github.com/evanmascitti/soiltestr>), which was developed using principles outlined within the tidyverse framework [@Wickham2019a].
...
The LL of mixtures containing the coarsest sand (2000-1000 &mu;m) were nearly identical to those predicted by the linear law, all the way up to 75% coarse addition.
...
Those containing coarse additions <250 μm had elevated plastic limits and began to diverge from the linear law at around 20% coarse addition. Mixtures containing coarse additions >250 μm still adhered closely to the linear law.
...
ecmfuns::insert_fig(stem = 'experiment-1-atterberg-limit-facets')
...
This finding aligns with those of @Dumbleton1966b,  @Sivapullaiah1985, @Barnes2013 and extends the concept to include particles >425 &mu;m.
...
ecmfuns::insert_fig(stem = 'experiment-1-estimated-toughness')


```


```{r load-estimated-t-max-values}

# for this one, there are no real calculations, so I am just doing it 
# here instead of storing in an external dynamic reference building file 

all_t_max_values <- readr::read_rds(
  file = here::here("ecmdata", "derived-data", "cleaned-rds-files", "experiment-1-estimated-toughness-values-cleaned-data.rds")
)

pure_clay_t_max <- unlist(all_t_max_values[all_t_max_values$coarse_size == '2.0-1.0' & dplyr::near(all_t_max_values$coarse_pct,  0), 't_max']) %>% 
  round(., digits = 1)

very_coarse_sand_t_max_at_70_pct_sand <- unlist(all_t_max_values[all_t_max_values$coarse_size == '2.0-1.0' & dplyr::near(all_t_max_values$coarse_pct,  0.70), 't_max']) %>% 
  round(., digits = 1)

# fine_sand_t_max_at_70_pct_sand <- unlist(all_t_max_values[all_t_max_values$coarse_size == '0.25-0.15', 't_max']) %>%
#   round(., digits = 1)
...
At &ge;60% mixtures having the same coarse addition mass had markedly different estimated $T_{max}$.
For example, adding 70% coarse additions in the 2000-1000 &mu;m fraction only reduced $T_{max}$ from `r pure_clay_t_max` to `r very_coarse_sand_t_max_at_70_pct_sand` kJ m^-3^, while the addition of 70% particles between 250-150 μm or 150-53 μm reduced  $T_{max}$ to zero.
...
format_scientific <- function(x, digits = 2){
  ...
  scientific_base <-format(x, digits = digits, scientific = TRUE)
  
  base <- stringr::str_extract(scientific_base, "[\\d\\.]+(?=e)")
  
  exponent <- as.numeric(stringr::str_extract(scientific_base, "\\d+$"))
  
  text_version <- paste0(base, " x 10^", exponent, "^")
  
#   output_string <- paste0(base, text_version)
  
  return(text_version)
  
}

formatted_largest_to_smallest_n_particles_factor <- format_scientific(particles_per_g_data$finest_sand_n_particles_per_gram)
...
specific gravity of 2.65, 1 gram of these particles comprises `r particles_per_g_data$coarsest_sand_n_particles_per_gram` individual grains. 
In contrast, the finest sand had a  D~50~ of `r particles_per_g_data$finest_sand_d_50_mm` mm, representing `r ` particles per gram - a factor of ~`r largest_to_smallest_n_particles_factor`.
...
In the present study, SSA for each coarse addition was computed using 2.65 Mg m^3^ for $\rho_p$ and taking the D~50~ of each material as the representative particle diameter.
...
Note that this definition of $w_{matrix}$ differs from that of @Barnes2013, who assumed that water associated only with particles <2&mu;m (Equation \@ref(eq:barnes-matrix-water-content)).
...
This is identical to the oversize correction and the linear law (Equations \@ref(eq:oversize-correction-formula) and \@ref(eq:linear-law-of-mixtures)).
...
ecmfuns::insert_fig('experiment-1-matrix-water-content-at-LL-and-PL')
...
All particle diameters of the three finest coarse additions fell below the maximum allowable diameter in standard test procedures (<425 &mu;m), while the other three coarse additions included particles which would normally be removed from the sample prior to testing.
...
Additional work could also evaluate the effect of particle shape and particle-size uniformity of coarse additions >425 &mu;m by producing mixtures with equivalent mean particle diameter but varied shape or uniformity.
