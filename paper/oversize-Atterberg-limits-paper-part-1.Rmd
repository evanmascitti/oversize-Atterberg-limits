...
The plasticity index (PI) is the difference between LL and PL [@ASTMD43182018].
...
These soils will likely have different LL and PL values; however, it will be unclear whether the differences are due to the nature of the fines, or simply because more sand 2000-425 &mu;m was removed from the soil having the finer sand gradation.
...
1. Can the Atterberg limit tests be successfully performed without removing "oversize" particles 2000-425 &mu;m?

1. How do LL and PL of soil mixtures containing oversize particles compare to those for mixtures containing only particles <425 &mu;m?
...
Gravity sedimentation plus centrifugation were used along with the the pipette method to compute the percent of the sample finer than 20, 5, 2, and 2 μm [@Gee2002]. 
...
A `Makefile` which runs the relevant analysis code is available at in the supplemental materials.
...
<!-- this is not rendered to respect the double-blind requirement -->
```{r end-of-doc-extras, child="here::here('paper/oversizeAttLims-paper-part-1-end-of-doc-extras.Rmd')", eval=F}
```
